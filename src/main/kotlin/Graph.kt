...
class Graph(matrix: MutableList<List<Double>>){
        ...
        for(i in matrix.indices){
            for(j in matrix[i].indices){
                ...
                if(i != j && matrix[i][j] != 0.0){
                    this[i].addIncident(j, arrayOf(matrix[i][j], 0.0))
                    this[j].addIncident(i, arrayOf(matrix[j][i], 0.0))
                }
            }
        }
    }
    ...
    class Vertex(n: Int){
        ...
        val id: Int = n
        val incidents = mutableMapOf<Int, Array<Double>>()
        var x = 0.0
            private set
        var y = 0.0
            private set
        ...
        fun addIncident(v: Int, w: Array<Double>) : Vertex = apply { incidents[v] = w }
